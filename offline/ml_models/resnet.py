...
class ResNet(nn.Module):
    def __init__(self, block, layers, x_dim=28, n_channels=1, n_classes=10):
        ...
        self.layer2 = self._make_layer(block, 32, layers[1], stride=2)
        self.layer3 = self._make_layer(block, 64, layers[2], stride=2)
        self.avgpool = nn.AvgPool2d(int(x_dim / 4), stride=1)
        self.fc = nn.Linear(64 * block.expansion, n_classes)
...
def resnet20(x_dim: int = 28, n_channels: int = 1, n_classes: int = 10, **kwargs):
    model = ResNet(BasicBlock, [1, 1, 1], x_dim, n_channels, n_classes, **kwargs)
    return model


def resnet20(x_dim: int = 28, n_channels: int = 1, n_classes: int = 10, **kwargs):
    model = ResNet(BasicBlock, [3, 3, 3], x_dim, n_channels, n_classes, **kwargs)
    return model


def resnet32(x_dim: int = 28, n_channels: int = 1, n_classes: int = 10, **kwargs):
    model = ResNet(BasicBlock, [5, 5, 5], x_dim, n_channels, n_classes, **kwargs)
    return model
