...
def main():
    ...
    parser.add_argument('--lr-gamma', type=float, default=0.02, metavar='N',
                        help='LR Decay Rate')
    parser.add_argument('-w', '--num_workers', metavar='N', type=int, help='# Worker threads to load data', default=5)
    ...
    for b in budgets:
        ...
        model_utils.train_model(model, transferset, model_dir, testset=testset, criterion_train=criterion_train,
                                checkpoint_suffix=checkpoint_suffix, device=device, restored=False, task=args.testdataset,
