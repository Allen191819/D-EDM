...
class JacobianAdversary:
    ...
    def __init__(self, blackbox, budget, model_adv_name, model_adv_pretrained, modelfamily, seedset, testset,
                 testset_name,
                 device, out_dir, batch_size=cfg.DEFAULT_BATCH_SIZE, train_epochs=50, kappa=400, tau=None, rho=6,
                 sigma=-1,
        ...
        model_adv = model_utils.train_model(model_adv, self.D, self.out_dir, num_workers=10,
                                            ...
                                            device=self.device, restored=True, task=self.testset_name, epochs=1,
                                            log_interval=500, lr=0.2, momentum=0.9, batch_size=self.batch_size,
    ...
    def get_transferset(self):
        ...
        while self.blackbox.call_count < self.budget:
            ...
            model_adv = model_utils.train_model(model_adv, self.D, self.out_dir, num_workers=10,
                                                checkpoint_suffix='.{}'.format(self.blackbox.call_count),
                                                device=self.device, restored=True, task=self.testset_name,
                                                epochs=self.train_epochs, log_interval=500, lr=0.1,
            ...
            if (self.blackbox.call_count >= self.budget) or ((self.rho is not None) and (rho_current >= self.rho)):
                ...
                model_adv = model_utils.train_model(model_adv, self.D, self.out_dir, num_workers=10,
                                                    ...
                                                    log_interval=500, lr=0.02, momentum=0.9, batch_size=self.batch_size,
...
def main():
    ...
    adversary = JacobianAdversary(blackbox, budget, model_adv_name, model_adv_pretrained, modelfamily, seedset,
                                  testset, testset_name, device, out_path, batch_size=batch_size,
